...
It loads the saved emergency card data by slug from the database and renders the same visual card as the protected version, but without any HeartGuard UI chrome (no nav, no dashboard links). Just the medical card.

At the top: "HeartGuard Emergency Medical Card" header
At the bottom: "This card was generated by HeartGuard (heartguard.app). Data source: CredibleMeds QTdrugs List."
