  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <MetricTile
          label="Heart Rate"
          value={metric.heartRate > 0 ? String(Math.round(metric.heartRate)) : '—'}
          unit="bpm"
        />
        <MetricTile
          label="HRV"
          value={metric.hrv > 0 ? String(Math.round(metric.hrv)) : '—'}
          unit="ms"
        />
        <MetricTile
          label="Resting HR"
          value={metric.restingHR > 0 ? String(Math.round(metric.restingHR)) : '—'}
          unit="bpm"
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <MetricTile
          label="Steps"
          value={metric.steps > 0 ? String(Math.round(metric.steps)) : '—'}
          unit="today"
        />
        <MetricTile
          label="Calories"
          value={metric.activeEnergy > 0 ? String(Math.round(metric.activeEnergy)) : '—'}
          unit="kcal"
        />
        <div className="flex flex-col items-center rounded-lg bg-surface px-3 py-2">
          <span className="text-[10px] font-medium text-text-secondary uppercase tracking-wide">
